...
trans_chase_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/trans_chase_df.csv')

#Dataframe for Bank of America (mergers and acquisitions)
trans_boa_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/trans_boa_df.csv')

#Dataframe for Wells Fargo (mergers and acquisitions)
trans_wf_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/trans_wf_df.csv')  

#Dataframe for Citibank (mergers and acquisitions)
trans_cb_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/trans_cb_df.csv')  

#---------------------------Dataframe for the ownership of the Big Four Banks---------------------
#Dataframe for the Giga Banks ownership of the Big Four Banks
giga_ownership_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/giga_banks_ownership.csv')

#----------------------------Dataframe for the consolidated assets of the Big Four----------------
#Dataframe for the linear regression on the Big Four banks
big_four_lin_reg=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/lin_reg.csv')

#Dataframe for the treemap of the assets of the Big Seven Banks
treemap_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/treemap_df_assets.csv')

line_df=pd.read_csv('https://raw.githubusercontent.com/juanchok12/Concentration-of-Banking/main/datasets/line_plot_all_banks.csv')
...
#========================Color styles for plots======================================================


#---------------------------Logo  colors for the Big Four Banks (historical stock data)--------------------------
...
#---------------------------Logo colors for the Big Four Banks-------------------------------------------

color_discrete_map_1={'Chase': '#0D5EAF',  # Blue
                      'Bank of America': '#CB0D1F',  # Red
...
#--------------------------Styles for Plotly Express plots--------------------------------------------
px.defaults.template='plotly_dark' #Sets the default template for the Plotly Express plots to 'plotly_dark'
...
#========================Linear Regression Plot for the Big Four Banks=======================================

#-------------------Developing the regression analysis------------------

'''Defiting the X and Y values for the linear regression'''
X=big_four_lin_reg['Numeric_Quarter'].values.reshape(-1,1) #.reshape(-1,1) reshapes the 1D array into a 2D array with one column, which is the expected input format for the X values in .fit()
Y=big_four_lin_reg['Assets'].values #The reason for using .values is that many machine learning algorithms, including those in scikit-learn, expect the input data to be Numpy arrays.

#Fit the model using the unique quarters
linear_model=LinearRegression()
#fit() method takes the X and y values as arguments
linear_model.fit(X,Y) #Fit the model using the unique quarters. #fig() expects a 2D array for the X values


#Predict y-values using the unique sorted quarters
y_regression=linear_model.predict(X) #Predict the y-values using the unique sorted quarters


'''Mapping the unique numeric quarters back to string quarters'''
#Create the mapping from 'Numeric_Quarter' to 'Quarter' using unique values
unique_quarter_map=pd.Series(big_four_lin_reg['Quarter'].values,
                                      index=big_four_lin_reg['Numeric_Quarter']).to_dict()

#Map the unique quarters back to string quarters
unique_mapped_x=[unique_quarter_map[q[0]] for q in X] #Map the unique quarters back to string quarters. Use the first element of each quarter

#Display the first few mapped values
unique_mapped_x[:5], y_regression[:5]  # Display the first few mapped values

#Create a new dataframe for the regression line data
regression_line_df=pd.DataFrame({'Quarter':unique_mapped_x, 'assets_linear':y_regression})


#Cobine the data from the melted_big_4_df and the regression_line_df dataframes
linear_reg_df=pd.concat([big_four_lin_reg, regression_line_df['assets_linear']], axis=1)
...
#Finding the slope for the regression line
m=model.coef_[0]
...
# Create scatter plot
fig_lin_reg2 = px.scatter(linear_reg_df, 
                         x=linear_reg_df['Quarter'], 
                         y=linear_reg_df['Assets'], 
                         color=linear_reg_df['Bank'], # Color by bank
                        color_discrete_map=color_discrete_map_1,# Use custom color map


                        ) # Use custom color map

# Update layout by adding title and axis labels
fig_lin_reg2.update_layout(title='Linear Regression of the Big Four Banks',
                          xaxis_title='Quarter',
                          yaxis_title='Assets (in Trillions)',
                          template='plotly_dark'
                          )


#Adding the regression model as an annotation to the plot
fig_lin_reg2.add_annotation(
...
fig_lin_reg2.add_trace(
    go.Scatter(x=linear_reg_df['Quarter'],
            y=linear_reg_df['assets_linear'], 
            mode='lines', 
...
#=========================Plotting the Treemap of the Big Four Banks=======================================
#Creating the treemap
fig_treemap=px.treemap(treemap_df,
                       path=['Parent','Bank'],
                       values='Consolidated Assets',
                       title='Consolidated Assets Share of the Big Four Banks',
                       labels={'Consolidated Assets':'Assets (Trillions'},
                       color='Bank',
                       color_discrete_map=color_discrete_map_1,
                       hover_data={'Percentage':True} #Include the percentage in the hover labels
)

#Update hover labels by using the hovertemplate attribute:
for trace in fig_treemap.data:
    trace.hovertemplate='<b>%{label}</b><br>Assets: %{value:.3s}<br>Percentage: %{customdata[0]}<br>'


#========================Plotting the line plot of the Big Four Banks vs. total assets of all large commercial banks=======================================

#Linear plotting of "Total Consolidated Assets" and "Big 7 Consolidated Assets" per quarter
fig_lin_total=px.line(line_df,
                         x='Quarter',
                         y=['Total Assets','Big Four Assets'],
                         title='Total Assets by All Large Commercial Banks vs. Big Four Banks',
                         labels={'value':'Consolidated Assets($)','variable':'Bank groups'},
                         template='plotly_dark',
                         hover_data={'Share of Consolidated Assets':True},
                         color_discrete_map={'Big Four Assets': '#583ead', 'Total Assets': '#246b3d'})


#Update hover labels by using the hovertemplate attribute:
for trace in fig_lin_total.data:
    if trace.name == 'Big Four Assets':
        trace.hovertemplate = '<b>%{x}</b><br>Assets: %{y:.3s}<br>Percentage: %{customdata[0]}<br><extra></extra>'
    else:
        trace.hovertemplate = '<b>%{x}</b><br>Assets (Trillions): %{y:.3s}<br><extra></extra>'
    trace.hoverinfo = "x+y+name" # Show both x and y values in the hover label
...
# Row for Dropdown and Toggle================================================
    ...
    <u>Github repository</u>: "Concentration-of-Banking." [Link](https://github.com/juanchok12/Concentration-of-Banking).
    ...
    ''', 
     ...
     style={'padding': '1px', 
            ...
            dcc.Tab(
                ...
                children=[
                    html.H1('Consolidated Assets of the Big Four Banks', style={'textAlign': 'center'}),
                    dmc.Grid(
                        gutter="xs",  # Adjust space between plots
                        children=[
                            dmc.Col(dcc.Graph(id='lin-reg-plot', figure=fig_lin_reg2, style={'width': '100%', 'height': '400px'}), md=6, sm=12),
                            dmc.Col(dcc.Graph(id='treemap-plot', figure=fig_treemap, style={'width': '100%', 'height': '400px'}), md=6, sm=12),
                            ...
                            dmc.Col(dcc.Graph(id='line-plot', figure=fig_lin_total, style={'width': '100%', 'height': '400px'}), md=6, sm=12),
                        ] 
                    ),
                    dcc.Markdown(
                        '''<u>Data source</u>: Large Commercial Banks. Board of Governors of the Federal Reserve System. [Link](https://www.federalreserve.gov/releases/lbr/).<br>
                        The Board of Governors of the Federal Reserve System publishes quarterly tables on the consolidated assets of all large commerical banks in the U.S.The banks are considered large if they have assets of $300 million or more.
                        'T' represents Trillions in USD.
                        ''',
